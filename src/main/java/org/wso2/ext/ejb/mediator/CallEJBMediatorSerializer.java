...
public class CallEJBMediatorSerializer extends AbstractMediatorSerializer {
    ...
    public OMElement serializeSpecificMediator(Mediator m) {
        ...
        if (!(m instanceof CallEJBMediator)) {
            handleException("An unsupported mediator was passed in for serialization : " +
                    m.getType());
            return null;
        }

        CallEJBMediator mediator = (CallEJBMediator) m;
        ...
        if (mediator.getJndiConfigRegPath() != null) {
            mediatorElem.addAttribute(fac.createOMAttribute(
                    EJBConstants.JNDI_CONFIG_REG_PATH, nullNS, mediator.getJndiConfigRegPath()));
        }
    ...
    public String getMediatorClassName() {
        return CallEJBMediator.class.getName();
    }

    private void handleException() {
        handleException("Invalid ext mediator was passed in for serialization.");
    }
}
